...
function AFWebAction(h, display, isEnabledCb, dataBody, actionCb) {
    ...
    this._renderVDom = function () {
        ...
        return h('form', { onsubmit: onSubmit }, [
            ...
            h('button', { type: 'submit', disabled: isPending || !isEnabled, style: { fontSize: '24px' } }, [ display ]),
            currentError
...
function AFWebGroup(h, bodyList) {
    ...
    this._renderVDom = function () {
        return h('div', bodyList.map(function (b, index) {
            return [ index === 0 ? null : h('hr'), b._renderVDom() ];
        }));
    };
}
